...
namespace Agenda
{
    public partial class FormAgenda2 : Form
    {
        ...
        public FormAgenda2(dynamic aux, List<compromisso>agenda, DataGridView DataGridComp, MonthCalendar monthCalendarDataInicio, MonthCalendar monthCalendarDatafim)
        {
           ...
           // textBoxLocal.Text = aux.Local;
            ...
            else if (aux is Lembrete)
            {
                ...
                comboBoxTerminar.SelectedItem = (EnumTerminarLembrete)aux.TempoFim;
            }
            /* if (aux.DiaLembrete.item1 == true)
            {
                comboBoxRepetir.SelectedItem = "Domingo";
            }
            else if (aux.DiaLembrete.item2 == true)
            {
                comboBoxRepetir.SelectedItem = "Segunda";
            }
            else if (aux.DiaLembrete.item3 == true)
            {
                comboBoxRepetir.SelectedItem = "Terça";
            }
            else if (aux.DiaLembrete.item4 == true)
            {
                comboBoxRepetir.SelectedItem = "Quarta";
            }
            else if (aux.DiaLembrete.item5 == true)
            {
                comboBoxRepetir.SelectedItem = "Quinta";
            }
            else if (aux.DiaLembrete.item6 == true)
            {
                comboBoxRepetir.SelectedItem = "Sexta";
            }
            else if (aux.DiaLembrete.item6 == true)
            {
                comboBoxRepetir.SelectedItem = "Sabado";
            }*/
        ...
        private void editarCompromisso_Click(object sender, EventArgs e)
        {
            ...
            aux.Descricao = textBoxDescrição.Text;
            aux.Datahorainicio = dateTimeDataInicio2.Value;
            aux.Datahorafim = dateTimePickerDataFim2.Value;
            
            if(aux is Tarefa)
            {
                aux.Prioridade = (Char)(EnumTarefa)Enum.Parse(typeof(EnumTarefa), comboBoxTarefa.SelectedItem.ToString());
                comboBoxTarefa.SelectedItem = (EnumTarefa)aux.Prioridade;
            }
             else if(aux is evento)
            {
